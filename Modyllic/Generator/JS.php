...
class Modyllic_Generator_JS {
    ...
    function start_funcdoc() {
        $this->indent();
        $this->add("/**\n");
        return $this;
    }
    function funcdoc($line) {
        $this->indent();
        $this->add(" * $line\n");
        return $this;
    }
    function end_funcdoc() {
        $this->indent();
        $this->add(" */\n");
        return $this;
    }
    ...
    function helpers($schema) {
        ...
        return $this->get_and_flush_js();
    }
    function get_and_flush_js() {
        $js = $this->js;
        $this->js = "";
        return $js;
    }
    ...
    function begin_func(Modyllic_Schema_Routine $routine) {
        $this->begin_cmd( 'function '.$routine->name.'(' )
               ->args($routine)
             ->end_cmd(') {');
        $this->begin_block();
        return $this;
    ...
    function args_docs_preamble(Modyllic_Schema_Routine $routine) {
        $this->funcdoc('@param function dbquery The database query function with a signature of (string sql,array bound,function callback) where the callback has a signature of (e,array rows)');
        return $this;
    }
    function args_list_docs(array $args) {
        foreach ($args as $arg) {
            $this->funcdoc( '@param '.$arg->type->to_sql().' '.$arg->name.' '.$arg->docs );
        }
        return $this;
    }
...
*/
    ...
    function args_map_docs(array $args) {
        $this->funcdoc( '@param object args Valid properties are:' );
        $max_len = 0;
        foreach ($args as $arg) {
            $max_len = max( $max_len, strlen($arg->name) );
        }
        foreach ($args as $arg) {
            $this->funcdoc( sprintf('       %-'.$max_len.'s => %s %s',$arg->name,$arg->type->to_sql(),$arg->docs ) );
        }
        return $this;
    }
    ...
    function func_returns_docs(Modyllic_Type $returns) {
        $this->funcdoc( "@param function callback Called with the result of the function with a signature of (e,".$returns->to_sql()." result)");
        return $this;
    }
    function proc_returns_docs(array $returns) {
        switch ($returns['type']) {
            case "ROW":
                ...
                break;
            case "COLUMN":
                $result = $returns['column'];
                break;
            case "LIST":
                $result = "array of ".$returns['column'];
                break;
            case "TABLE":
                $result = "array rows";
                break;
            case "MAP":
                $result = "object ".$returns['key']." => ".$returns['value'];
                break;
            case "STH":
            case "PROMISE":
                $result = "function fetch(callback(e,row,fetch)) A function that can be called with a callback that will be called with either an error or the next row and a new 'fetch new row' callback";
                break;
            case "NONE":
                break;
            default:
        ...
        $this->funcdoc( "@param function callback Called with the result of the procedure with a signature of (e,$result)");
        return $this;
    }
